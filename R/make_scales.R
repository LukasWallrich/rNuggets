  ...
  dplyr::select(dplyr::one_of(scale_items)) %>%
  {
    ...
    if (print_desc) {
        ...
        if (length(reversed) > 0) {
            print(paste(c("The following items are reverse coded: ", reversed), sep = ", ",
                collapse = ", "))
            print(paste("The min and max used for reverse coding:", min(scale_vals, na.rm = T),
...
make_mult_scales <- function(df, items, reversed = NULL, two_items_reliability = c("spearman_brown",
    ...
    if (!is.null(reversed)) {
        ...
        if (length(scales_rev) > 0) {
            print(paste0("The following scales will be calculated with specified reverse coding: ",
    ...
    if (length(scales_n_rev) > 0) {
        ...
        print(paste0("The following scales will be calculated without reverse coding: ",
...
svy_make_scale <- function(df, scale_items, scale_name, print_hist = T, scale_title = scale_name,
    ...
    if (!is.null(reversed)) {
        ...
        scale_items_num <- c(setdiff(scale_items_num, reversed_num), paste0(reversed_num,
            "r"))
    ...
    if (print_hist) {
        ...
        for (i in 1:length(hist_vars)) {
            x <- as.data.frame(survey::svytable(as.formula(paste0("~round(", hist_vars[i], ")")),
                df))
            x$var <- stringr::str_split(names(x[1]), "\\.")[[1]][2]
            colnames(x)[1] <- "val"
            df2 %<>% rbind(x)
        }
        print(ggplot2::ggplot(df2, ggplot2::aes(.data$val, y = .data$Freq)) + ggplot2::geom_bar(stat = "identity") + ggplot2::facet_wrap(~var))
    }
    return(df)
}
